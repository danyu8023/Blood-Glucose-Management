...
onMounted(async () => {
  ...
  try {
    ...
    if (dashboard.latestGlucose) store.commit('records/setGlucose', [{ value: Number(dashboard.latestGlucose.value), period: ({ fasting: '空腹', pre_meal: '餐前', post_meal: '餐后', bedtime: '睡前' } as Record<string, string>)[dashboard.latestGlucose.period] || dashboard.latestGlucose.period, time: '刚刚', note: String(dashboard.latestGlucose.note || '') }])
