<template>
  <section class="page">
    ...
    <template v-if="loggedIn">
      <div class="glass rounded-[26px] p-5 mb-4 flex items-center gap-4"><span class="avatar avatar-lg" aria-hidden="true">{{ name.slice(0, 1) }}</span><div class="flex-1"><h2 class="text-xl font-semibold">{{ name }}</h2><p class="muted text-sm mt-1">{{ diabetesType }} · 已登录</p><p class="text-xs text-[#34c759] mt-2">数据已同步</p></div></div>
